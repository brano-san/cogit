...
describe("the right columns", () => {
  ...
  it("take their widths, a gap before each and the row's padding", () => {
    ...
    expect(width).toBe(
      COLUMN_WIDTH.author + COLUMN_WIDTH.avatar + timeWidth("date") + COLUMN_WIDTH.hash + 4 * 6 + 12,
    );
  });
  ...
  it("make room for a longer time format", () => {
    expect(rightColumnsWidth({ ...spec, time: "dateTime" })).toBeGreaterThan(rightColumnsWidth(spec));
...
describe("graphTime", () => {
  ...
  it("writes the formats the graph offers", () => {
    ...
    expect(graphTime(noon, 0, now, "date")).toBe("Thursday");
    expect(graphTime(noon, 180, now, "dateTime")).toBe("Thursday 15:05");
  });
