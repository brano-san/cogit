...
import { GRAPH_MIN_SUBJECT_CHARS, graphPanelMinWidth, subjectMinWidth } from "./graph-panel";
...
describe("the Graph panel's minimum width", () => {
  ...
  it("reserves the same characters for the subject in every row", () => {
    expect(subjectMinWidth()).toBe("35ch");
  });
});
