...
describe("menuStatePusher", () => {
  ...
  // A rebuilt bar starts with every tick cleared, and muda flips a tick on a click.
  it("sends the same state again when told the bar may not show it", async () => {
    const { send, push } = pusher();
    push(["fetch"], ["output"]);
    await vi.advanceTimersByTimeAsync(MENU_STATE_DELAY_MS);
    push(["fetch"], ["output"], { resend: true });
    await vi.advanceTimersByTimeAsync(MENU_STATE_DELAY_MS);
