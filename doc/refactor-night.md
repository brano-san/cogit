...
- [x] F3-01…06, F3-08 мёртвый фронт
  > Итог: польза: −85 строк, из них 10 CSS-правил под классы, которых подсветка не выдаёт
  > (проверено по dist `@lezer/highlight`). Удалены: `Tooltip.svelte` (Toolbar держит `data-
  > tip` на слоте — у выключенной кнопки нет событий указателя, обёртка сломала бы
  > подсказку), IPC-обёртки `deleteUntracked`, `setUpstream`, `diffFiles`, `fileBefore` и
  > реэкспорт типов с устаревшим комментарием, `sourceOf`, `sameLocation`, `forgetSettings`.
  > Не сделано: F3-02 (`links.ts`) и F3-06 (экспорты только для своих тестов) — удаление
  > потянуло бы тесты. **На решение:** Rust-команды `delete_untracked`, `set_upstream`,
  > `diff_files` (пакетный дифф из 08-diff-engine §9), `file_before` теперь без единого
  > вызова с фронта — удалить целиком или подключить.
- [x] D4-01, 02, 05, 06, 08…12 дубли Rust
  > Итог: польза — один источник вместо копий, в которых правка расходится: D4-02 — общие
  > `path_of` и `LogHeader` для двух читателей `log -L`; D4-05 — `git_command()` с общим
  > ядром (окно, prompt, LC_ALL, очистка GIT_*) для `base_command` и `bare_git`; D4-06 —
  > один `bash()` для хуков и проверок на общей `CREATE_NO_WINDOW`; D4-09 — `elapsed_ms`
  > вместо пяти копий. Не сделано: D4-01 (base64) — нужна новая зависимость или связь
  > крейтов, выигрыш ~25 строк; D4-08 — тест потока окон ищет `child_window::` в теле
  > команды, хелпер ослабил бы его; D4-10, D4-11, D4-12 — по 1–3 строки, пользы нет; D4-07 —
  > меняет запуск терминала, **на решение**.
- [x] F4-04…10 дубли фронта
  > Итог: F4-07 сделан: 24 ручных `.slice(0, 7)` в 15 файлах и частный `short` в RefActions
  > → `shortOid` (длина задана в одном месте); три места с `?.` оставлены — замена изменила
  > бы вывод при null. Не сделано: F4-04 (меню литералами → lib с тестами) и F4-06 (буфер
  > обмена — реализации по-разному сообщают об ошибке, объединение меняет поведение) —
  > отложены по времени; F4-05, F4-08, F4-09, F4-10 — по 2–6 строк, польза не стоит риска
  > ночью.
- [x] F5-01, F5-02, S5-06, S5-16 упрощение
  > Итог: не сделано: F5-01 — «Fast-forward» зовёт обычный merge (в `MergeOptions` нет ff-
...
- [x] M6-01 commands/mod.rs (перенос)
  ...
  > команд), `hooks.rs` (9), `branches.rs` (8), `network.rs` (8), `conflicts.rs` (7),
  > `stash.rs` (6), `flow.rs` (4), `presets.rs` (4), `avatars.rs` (3) и тесты модуля в
  > `tests.rs`; в `lib.rs` поменялись только пути регистрации. 2612 → 1426 строк. Осталось в
  > mod.rs: открытие/история/дифф/blame/rewrite и настройки — следующий шаг, если нужен.
  > Проверки по исходнику (главный поток, окна, очередь) обходят все файлы каталога —
  > перенос их не ослабил (103 теста).

## Фаза 3 — итог

- [x] Полный прогон тестов после правок
  > Итог: nextest 1678 ✓ / 1 skipped (было 1645 — плюс 33 новых теста); `cargo test -p cogit
  > --lib` 103 ✓ (было 100); vitest 1671 ✓ в 123 файлах (было 1653 в 121); svelte-check 0/0;
  > clippy `-D warnings` чисто. Флапов тестов с замером времени не было.
- [x] Бенчмарк: A/B `night-base` (2e4ef36) против `night-final` в одной сессии
  > Итог: `--ab night-base,night-final --rounds 6 --hidden --cores 16-31`, 40,9 мин,
  > `doc/benchmarks/2026-09-24-night-ab.json`. Все 85 сценариев — «same» (порог: интервал
  > целиком ниже 0,97 или выше 1,03). Регрессий нет — откатывать нечего. Ближе всех к
  > «быстрее» graph.tick-tags и graph.tick-branch на large (0,966 [0,948; 0,983], 0,965
  > [0,948; 0,981]); самые широкие интервалы — app.close medium (1,58 [0,64; 2,33], 19 → 30
  > мс, шум закрытия процесса) и files.status large (1,14 [0,88; 1,14]); оба накрывают 1.
  > Ускорения и не ждалось: правки на горячих путях сознательно отложены.
- [x] Итоговые таблицы
  > Итог: см. ниже; cron-подсказка удалена.

### По категориям

| Категория | Найдено | Исправлено | Отложено / не сделано |
|---|---|---|---|
| 1. Гонки | 35 (фронт 18, бэк 17) | 24 | 11: C1F-11, 15, 16, 18; C1-01, 04, 05, 09, 12, 14, 17 |
| 2. Баги и несостыковки | 31 (+ D4-03) | 20 в коде + 5 в документации | 3: B-17, B-25, B-31; B-12 уже работал; 3 дубля других пунктов |
| 3. Неиспользуемое | 17 | 10 | 7: D3-04, D3-06, F3-02, F3-06 (удаление потянуло бы тесты), D3-08/F3-07 (на решение), D3-09 (без пользы) |
| 4. Дубли | 24 | 7 (D4-02…06, 09, F4-07) | 17: без пользы ночью или меняют поведение (D4-07, F4-02, F4-03, F4-11, F4-12) |
| 5. Упрощение | 19 | 1 (S5-15) | 18: горячий путь — только с A/B (S5-01…05, 07…12), остальное косметика |
| 6. Модули | 4 | 3, частично | M6-04 (тестовый крейт, польза мала) |

### Найденные баги и гонки — по коммитам

Каждый `fix` — с тестом, который воспроизводит баг (падал до правки), кроме отмеченных.

- 51d5421 — watcher-refresh во время открытия отменял открытие другого репозитория (C1F-01)
- 5597d97 — перезагрузки для репозитория, с которого уже ушли: эпоха в `repository` (C1F-02/03/04/08)
- f510803 — поздние ответы для ушедшего репозитория в 8 сторах и 2 местах App (C1F-05/06/07/14)
- 64772c6 — картинки прошлого файла под диффом следующего (C1F-09)
- bdd6567 — «Назад» в Investigate перезаписывался blame покинутого места (C1F-10)
- a2a35d2 — тело одного хука в редакторе другого; Save писал его туда (C1F-12)
- efc8cba — старый список репозиториев перезаписывал новый и сессию (C1F-13)
- 8b8ae18 — выбор коммита не убирал stash из Files (C1F-17; **без юнит-теста**, проводка App)
- 983bfa6 — переполнение шины навсегда глушило события в webview (C1-02)
- 08ea234 — выключение аватаров держало лок читателей до ~10 с (C1-03)
- ae453b0 — кэш строки репозитория сохранял прочитанное во время изменения (C1-06)
- 0ce05b3 — один путь, открытый дважды одновременно, давал два id и два вотчера (C1-07)
- 30c562c — запись config, хуков, exec-бита и корзины мимо очереди (C1-08)
- f9522c1 — список репозиториев читался на async-воркере (C1-10)
- 8fc413e — отмена поисков на каждое нажатие крестика, даже отменённое (C1-11)
- ebf587d — запись Undo для закрытого репозитория (C1-13)
- b590ee5 — параллельные dry-run commit-msg удаляли файл сообщения друг у друга (C1-15)
- ff81dcf — скан репозиториев шёл до конца после закрытия диалога (C1-16)
- 7903fd3 — Undo merge/rebase/cherry-pick всегда падал «branch already exists» (B-01)
- 784f39f — discard/rollback выбрасывали файлы, когда резервный stash не удался (B-02/03, R-284)
- 43a830d — битый settings.json затирался одной настройкой (B-04)
- 6684b60 — `.gitattributes` не в UTF-8 перезаписывался одной строкой (B-05)
- 4526be2 — merge внутри `rebase -r` определялся как merge (B-07)
- bd12b89 — хуки и refs linked worktree искались не в common dir (B-08/09)
- 1493aa7 — split off не принимал файлы с не-ASCII именами (B-13)
- 30a6f4e — пути шли в git как шаблоны: discard `test[1].txt` удалял `test1.txt` (B-14)
- 66c0edd — hard reset отказывал, если сдвинут только сабмодуль (найден тестом к B-15)
- 0bfab59 — Undo удаления аннотированного тега создавал лёгкий (B-16)
- 53acb4b — Undo finish flow восстанавливал ветку на коммите одноимённого тега (B-18)
- 2553937 — stderr сети с U+FFFD на границе чтения (B-19)
- c795679 — часть пароля с `@` попадала в журнал (B-20)
- 38bcbf1 — занятый файл показывался удалённым (B-21)
- 90ee287 — многокоммитный cherry-pick после ручного Commit выглядел завершённым (B-26)
- 9194ea9 — Investigate терял шаги из середины длинной истории (D4-03: 9 из 60)
- 12efc51, 60440d3 — ошибки вотчера и отката split теперь в логе (B-10, B-24; только лог, без теста)

### Сознательно не сделано и почему

- **Правки скорости на горячих путях** (S5-01…05, 07…12: аллокации в графе, разборе коммитов,
  диффе): правило проекта — только с A/B на каждую; ночью на это нет времени, а без замера
  они не принимаются.
- **Недетерминированные гонки без воспроизводящего теста** — C1-04 (индекс аватаров: 8×100
  параллельных сохранений не потеряли ни записи), C1-17 (микросекундное окно); баг-фикс без
  теста по заданию не принимается.
- **Удаление мёртвого кода вместе с тестами** (D3-04, D3-06, F3-02, F3-06): тесты не удаляю.
- **D4-08** (хелпер открытия окна): тест потока окон ищет `child_window::` в теле команды, и
  хелпер сделал бы его слепым.
- **C1F-15, C1F-18**: правка только в разметке, тест был бы по разметке — против правила фронта.
- **B-12**: уже работало (git 2.51 не экранирует пути в `log -L`), добавлен тест-страховка.
- Разнесение модулей сделано частично: вынесены независимые группы; реестр репозиториев в
  app_state и строка меню/раскладка клавиш держатся на приватных полях — перенос потребовал бы
  менять видимость.

### Требует решения пользователя

1. **Чужой stash**: `stash@{1}` — ваш «botched bulk comment strip», нетронут; `stash@{0}` —
   моё случайное его наложение (см. «Режим»), его можно удалить `git stash drop stash@{0}`.
2. **Настройка `gitPath`** (D3-08/F3-07) сохраняется, но ничего не делает: подключить в runner
   или убрать из панели.
3. **Rust-команды без вызовов с фронта**: `delete_untracked`, `set_upstream`, `diff_files`
   (пакетный дифф из 08-diff-engine §9), `file_before` — удалить или подключить.
4. **INV-06** против рекурсивного наблюдения корня (R-285): сузить или переписать инвариант.
5. **Тихое окно вотчера** (C1-12): внешнее сохранение в ~400 мс после нашей мутации теряется;
   исправление вернёт двойную перезагрузку (R-25).
6. **Большие discard** (R-284): теперь отказывают, если нельзя сделать резервный stash —
   нужен ли stash порциями.
7. **Порядок очереди** (C1-01: место берётся при первом poll, не при invoke) и **очередь по
   git-dir** (C1-09: worktree и основной репозиторий пишут параллельно) — изменения дизайна.
8. **Закрытие репозитория не ждёт очередь** (C1-13, вторая половина): мутации в очереди
   падают с RepoNotFound.
9. **C1-17** — поменять две строки в `settle` (release раньше emit) без теста — да/нет.
10. **C1F-17** (stash и выбор коммита) — оставить без юнит-теста или откатить.
11. **Fast-forward** из перетаскивания (F5-01) делает обычный merge — нужен ли `--ff-only`.
12. **Токен в `-c http.extraHeader`** (B-31) виден в командной строке и уходит на все хосты.
13. **CLAUDE.md**: «Raw git output is never truncated», а окно обрезает вывод (лог хранит всё).
14. **Undo для reset mixed/soft, edit_author, rename_tag, abort** (B-17) — новое поведение.
