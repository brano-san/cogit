...
| Команда | Вход | Выход | Модуль |
|---|---|---|---|
| `load_commits` | `repo, cursor: Option<String>, limit, channel: Channel<CommitChunk>` | `()` | M4 |
...
1. UI создаёт `Channel<CommitChunk>` и вызывает команду с `cursor = null`.
2. Бэкенд обходит историю и шлёт чанки по 100–200 коммитов; каждый чанк уже содержит
   готовую раскладку дорожек для своих строк.
3. Последний чанк несёт `is_last: true` и `next_cursor: null`.
4. При скролле UI вызывает команду снова с `next_cursor` из последнего чанка.
...
**Отмена.** Пользователь может переключить репозиторий во время загрузки. Каждый запуск
получает `generation: u64`; при смене репозитория счётчик увеличивается, и чанки с устаревшим
`generation` на фронтенде отбрасываются, а на бэкенде обход прерывается.
