...
`first_parent` тоже не предикат: обход идёт только по первым родителям, как
`git log --first-parent`. Слитая сторона в обход не попадает; линия слияния к ней
кончается стрелкой, как линия к отфильтрованному родителю (R-161). Граф остаётся графом —
`filters_rows()` его не считает. На фронтенде — `graph.firstParent` (настройка
`graphFirstParent`), подмешивается в каждую загрузку, как `visibleRefs`.
...
2. Бэкенд обходит историю отмеченных ссылок чанками по 200 коммитов, раскладывает каждый
   ...
   хэш всех ссылок и HEAD, без чтения коммитов) отвечается сразу одним сообщением
