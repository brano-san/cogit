...
ПКМ по A → `Close Repository` — на экране следующий открытый репозиторий, не S, и S не
появляется в списке отдельной записью (в том числе после правки файла в S и `Refresh`
до закрытия, R-543).
