...
(группы панели), `Pin`, `Rename…`, `Remove…`. Пока меню открыто, его строка (или узел
submodule) обведена, а выбранный репозиторий остаётся прежним (R-545). У строки с меткой `missing` — открытой или
