...
- [ ] Фаза 1 — аудит (workflow 1): `doc/audit/<ключ>.md`, два прохода на область.
- [ ] Фаза 2 — проверка находок другим агентом (critical/high — двумя, спор — третьим), слияние
  дублей, владельцы, `findings.md`.
- [ ] Фаза 3 — статусы 310 фич (`тест` / `автоматизация` / `только по коду` / `сломано`),
  тесты для критичных путей без тестов, полный прогон и бенчмарк — база.
...
- 25.09 02:05 — снимок перед лимитом. Готово: проход 1 всех 10 областей (256 находок: git-engine
  ...
  фаза 4.
- 25.09 — автопродолжение: cron `cd4b5551` (:07 и :37 каждого часа) проверяет workflow и
