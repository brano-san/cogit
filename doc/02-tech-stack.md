...
Единственное исключение — аватары: Gravatar это обычный GET, к git отношения не имеющий.
...
| ~~`tauri-plugin-shell`~~ | — | **Не взят**, см. ниже: терминал запускается из Rust |
...
### Почему `tauri-plugin-shell` не взят

Он планировался под «Open in Terminal» и оказался не нужен. Плагин даёт управление
sidecar-процессами и скоупы разрешений; здесь требуется один `spawn`. Цена — разрешение
`shell:allow-execute` в capabilities, то есть право запускать процессы **из вебвью**.
В клиенте, который рендерит содержимое чужих репозиториев, это лишняя поверхность ради
одной кнопки.

Вместо него — команда `open_in_terminal` в `src-tauri` поверх `std::process::Command`.
Выбор программы вынесен в `app_state::terminal` как данные и покрыт тестами на каждый
вариант. Путь передаётся **рабочей директорией** процесса; аргументом — только тем
программам, которые унаследованную директорию игнорируют (`wt -d`, `open -a Terminal`).
