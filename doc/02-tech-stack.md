...
| ~~`tauri-plugin-store`~~ | — | **Снят**, см. ниже: ронял релиз на старте |
...
| `windows-registry` | 0.6 | Выпуск Windows (`24H2`) и номер обновления для About; уже был в дереве через `hyper-util`, см. [R-171](12-risks.md) |
| `sysinfo` | 0.39 | Кроме замера памяти WebView2 — название и версия ОС в About на Linux и macOS (`app_state`) |

### Почему `tauri-plugin-store` снят

Он повреждал кучу вскоре после запуска, и релизная сборка умирала за несколько секунд.
Локализовано перебором: без плагина пять запусков из пяти живы, с ним — пять падений
из пяти. Разбор целиком в [12-risks.md, R-81](12-risks.md).

Замены не искали. Настройки нужны Rust **раньше, чем появляется окно**: на старте по ним
строится меню и настраивается логгер. Хранилище во владении фронтенда для этого не
подходит в принципе, и это было видно ещё до падения — Rust читал тот же файл сам, мимо
плагина. Теперь владелец один: `app_state::settings` плюс команды `read_settings` и
`write_setting`, около семидесяти строк на оба конца.
...
### Сборочный крейт `build_info`

Не зависимость, а свой крейт workspace: чистые функции для `src-tauri/build.rs` — обход
`cargo metadata` до списка крейтов, которые реально линкуются в приложение, и запуск
процесса с таймаутом. У build-скрипта не бывает своих тестов, поэтому логика вынесена
сюда. Зависит только от `serde_json` ([R-173](12-risks.md)).
...
|---|---|---|
| `insta` | 1.48 | Snapshot-тесты дифов |
