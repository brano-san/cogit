...
3. **Только тёмная тема** в v1.0. Но токены уже разделены на слои, чтобы светлая тема
   не потребовала переписывания компонентов.
...
цветом `--status-ref` с подчёркиванием при наведении.
...
- [ ] Ни одного литерального цвета — только токены
- [ ] Отступы взяты из шкалы `--sp-*`
- [ ] Высоты строк соответствуют `--h-*`
- [ ] Выделение показано не только цветом (полоса, иконка или подпись)
- [ ] Фокус с клавиатуры виден
- [ ] Числовые колонки используют `tabular-nums`
- [ ] Проверено при 125% и 150% масштабирования системы
- [ ] Пустое состояние панели оформлено
- [ ] Поведение при очень длинном имени файла/ветки — обрезка с многоточием и тултипом
