...
- [x] 5.1 FS-5-01, FC-5-01, FC-5-02, FA-5-03, GE-5-02 (не горячие); горячие (FA-5-01, FA-5-02,
  GE-5-01, AS-5-01, AS-5-02, FC-4-01) — только с A/B
  > Итог: один коммит: FC-5-02 — контекстное меню графа без открытого репозитория ничего не
  > выбирает, вместо обращения к репозиторию с поддельным id 0. Не делал: FS-5-01 (сделать,
  > чтобы worktree-мутации бросали ошибку) — меняет путь и заголовок уведомления и ручные
  > проверки `worktree.error` в RefActions и commitStaged, это поведение; FC-5-01 и FA-5-03
  > — перестройка компонентов без пользы для поведения; GE-5-02 и все горячие (FA-5-01,
  > FA-5-02, GE-5-01, AS-5-01, AS-5-02, FC-4-01) — меняют скорость, по правилу проекта
  > только с A/B на каждую правку; в этой итерации не брал.

### Категория 6 — модули

- [x] 6.1 ST-6-01, GE-6-01, GE-6-02, FS-6-01, FS-6-02, FC-6-01, FC-6-02, FA-6-01 — чистый перенос
  > Итог: три переноса без логики, каждый отдельным коммитом: ST-6-01 типы событий и их
  > форвардер — в `src-tauri/src/events.rs` (lib.rs 499 → 407, биндинги не изменились, пути
  > `crate::RepoChanged` и др. сохранены реэкспортом); GE-6-02 Ignore и Delete — из модуля
  > статуса в file_ops.rs; GE-6-01 hooks.rs (549) разделён: запуск хука и команда проверки —
  > hook_run.rs, журнал `--no-verify` — bypass.rs, `commit_template` — рядом с `commit()`;
  > трём методам hooks.rs понадобилась видимость `pub(crate)`. Не делал: FS-6-01 и FS-6-02
  > (ipc/index.ts и toolbar.ts — тонкие обёртки и таблицы, польза деления мала против риска
  > циклических импортов и моков в тестах); FC-6-01, FC-6-02, FA-6-01 — это не перенос, а
