...
use diff_engine::{DiffOptions, FileDiff, LineEnding, PatchRequest, build_patch, diff_text};
...
fn selecting_one_insert_leaves_the_others_out() {
    ...
    let patch = build_patch(&request(old, new, &[(2, false)])).unwrap();
...
fn an_unselected_delete_becomes_context_rather_than_disappearing() {
    ...
    let patch = build_patch(&request(old, new, &[(1, true)])).unwrap();
...
fn staging_only_deletions_while_insertions_exist_in_the_same_hunk() {
    ...
    let patch = build_patch(&request(old, new, &[(2, true)])).unwrap();
